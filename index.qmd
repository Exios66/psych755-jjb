...
abstract: |
  ...
  Communication Apprehension (PRCA-24) group and interpersonal subscales (range 6–30),
  ...
  transit at cross-validated receiver-operating-characteristic AUC (CV ROC-AUC) = 0.762,
...
**Table.** Persona prompt tiers used across all live LLM baselines (versions v1/v2/v3), with the classical tabular suite scored on the same five core tiers plus the three v3 ablations.
...
**Table.** Head-to-head pooled metrics across all prompt-v1 tiers (*N* = 241 × 5).
...
**Table.** Qualitative RQ1–RQ3 response pattern per model across the prompt-v1 ladder.
...
**Table.** Pooled v2 metrics across the five tiers, with transit-tier IP MAE per model.
...
**Table.** Pooled v3 metrics across all eight tiers.
...
**Table.** `full`-tier performance by model among live prompt-v1 runs.
...
**Table.** Thematic sample of free-response `full`-tier voice content (paraphrased / lightly excerpted).
...
**Table.** RF versus DeepSeek v2 MAE and band metrics by persona tier.
...
RF improves **monotonically** as tiers accumulate (MAE 5.47 → 4.48; macro-F1 0.31 → 0.42; band accuracy 0.37 → 0.51). DeepSeek v2 does not: `employment` is its *worst* tier (5.47), `transit` (5.03) barely beats `demos` (5.08), and its band metrics are flat at ~0.25–0.36 — below RF at every tier (@fig-f1-ml-vs-llm).
...
**Table.** Incremental change in Group-CA MAE as each persona tier is added, RF versus DeepSeek v2.
...
**Table.** Predicted-score profiles inside each estimated band for Group CA at the `transit` tier.
...
**Table.** Band discrimination at the `transit` tier: accuracy, distance, and AUC low-vs-high.
...
**Table.** Top-six TreeSHAP attributions for true Group CA versus surrogate-SHAP attributions for DeepSeek v2 outputs.
...
Ride-share days (Q28) and employment drive true-CA prediction, while the persona agent's outputs are dominated by **geolocation and age** — over-weighting place/demographic cues relative to the mobility/employment mix that predicts the actual PRCA scores (@fig-shap-ml-vs-llm). Mean |SHAP| magnitudes are also approximately 7× smaller for the surrogate, so no single profile feature dominates the agent's scores.
...
**Table.** Group-CA MAE by demographic slice across v2 tiers for DeepSeek-R1-Distill-Llama-8B.
...
Welch tests on the matched cohort show lower CA among regular riders ([memo](memos/transit_riders_ca.md); @fig-transit-ca-means):

**Table.** Welch Contrasts for Regular Versus Non-Regular Public-Transit Riders on PRCA Subscales
...
On the same outcome, a lat/long Random Forest yields cross-validated receiver-operating-characteristic AUC (CV ROC-AUC) = **0.551** (country-only AUC = **0.549**; chance = **0.500**; @fig-geo-ca-auc; [memo](memos/geo_predicts_transit.md)). A Random Forest on group + interpersonal CA yields CV ROC-AUC = **0.590** (group-only = **0.555**; interpersonal-only = **0.506**; out-of-fold confusion: true negatives = 76, false positives = 64, false negatives = 40, true positives = 61) ([memo](memos/ca_scores_predict_transit.md)).
...
@fig-q27-q28-prev, @fig-q27-q28-auc, and @tbl-q27-q28-auc summarize the Q27/Q28 reverse-prediction results ([memo](memos/q27_q28_predict_transit.md)). **Q28** (ride-share days) shows a monotonic prevalence gradient from **15.7%** regular (Never) to **94.1%** regular (8+ days/month) and CV ROC-AUC = **0.762** (average precision [AP] = 0.689; balanced accuracy = 0.730; F1 = 0.702). **Q27** (rides on a typical transit day) yields CV ROC-AUC = **0.589**. The joint Q27+Q28 forest reaches AUC = **0.761** (Δ = −0.001 vs Q28 alone).
...
**Table.** Headline CV ROC-AUC for the eight wave-2 follow-up experiments (seeded `--seed 42`, inner join).
...
**Table.** Unified Results Across the Primary, Secondary, and Focus Question Families on the Matched Cohort
