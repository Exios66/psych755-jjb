...
abstract: |
  ...
  self-reported communication apprehension (CA). Using McCroskey’s PRCA group and
  interpersonal subscales (range 6–30), we join Prolific demographics to Qualtrics
  ...
  cross-validated tabular suite (Ridge, Elastic Net, k-NN, Random Forest, XGBoost, MLP).
  Three primary research questions (RQ1–RQ3) examine whether employment status,
  transportation-use cues, and joint geo/employment/transit context improve absolute
  CA prediction error — under either family — and how prediction error distributes across
  demographic and contextual groups. Five secondary research questions (S1–S5) reverse the
  predictive direction on the same matched sample, asking how CA, place, and mobility
  self-reports relate to regular public-transit use (Q26 weekly-or-more) under classical
  ...
  Random Forest does, shows near-chance low-vs-high band discrimination (transit AUC 0.53
  group / 0.49 interpersonal vs RF 0.72 / 0.74), and never emits the high-CA band for
  Group CA; surrogate
  SHAP on its outputs shows the agent over-weights geolocation and age relative to the
...
![Cross-model pooled MAE and band accuracy by prompt version — prompt-v1 (v1), signal-first v2, and 8-tier v3 (greedy) vLLM exports](memos/figures/vllm_v1_cross_model_memo.png){#fig-vllm-v1-cross}
...
*Table.* Pooled v3 metrics across all eight tiers. The 8-tier pooling mixes the collapsed `transit`/`full` tiers with stable ablations, so pooled IP is pulled back toward v1 (5.76). Llama-3.2-3B base remained a 0% JSON parse (as in v1) and is excluded.
...
![Llama-3.1 vs DeepSeek interpersonal MAE by tier across prompt-v1 (v1), v2, and v3 (greedy) exports](memos/figures/vllm_v1_llama31_ip_collapse.png){#fig-llama31-ip-collapse}
...
RF improves **monotonically** as tiers accumulate (MAE 5.47 → 4.48; macro-F1 0.31 → 0.42; band accuracy 0.37 → 0.51). DeepSeek v2 does not: `employment` is its *worst* tier (5.47), `transit` (5.03) barely beats `demos` (5.08), and its band metrics are flat at ~0.25–0.36 — below RF at every tier.

![Band macro-F1 by tier — RF vs DeepSeek v2](memos/figures/fig_f1_ml_vs_llm.png){#fig-f1-ml-vs-llm}
...
|---|---:|---:|---:|---:|---:|---:|
| RF | low (6–13) | 96 | 11.95 (8.6–13.5) | 12.10 | 3.49 | 0.75 |
| RF | moderate (14–19) | 133 | 15.92 (13.5–19.5) | 16.30 | 5.31 | 0.31 |
| RF | high (20–30) | 12 | 21.12 (19.8–24.6) | 16.17 | 7.17 | 0.25 |
...
![Low-vs-high ROC-AUC by tier — RF vs DeepSeek v2](memos/figures/fig_band_discrimination_auc.png){#fig-band-discrimination-auc}

RF's low-vs-high AUC climbs from ~0.49 (`demos`) to **0.72–0.74** (`transit`); DeepSeek v2 stays near chance (0.45–0.53) at every tier. The LLM's error is **band-insensitive**: its scores hug the cohort mean, so absolute error is bounded while rank recovery fails. **MAE alone therefore understates the LLM's failure to recover the trait distribution** — band F1, band distance, and low-vs-high AUC belong alongside MAE in digital-twin evaluation.

### Feature attributions: TreeSHAP on true CA vs surrogate on LLM outputs

TreeSHAP mean \|SHAP| at the `transit` tier (predicting true Group CA) versus a surrogate RF fit to DeepSeek v2's own outputs (surrogate \\(R^2 = 0.74\\); tables: `shap_ml_group_raw.csv`, `shap_llm_surrogate_group_raw.csv`):

| Rank | RF → true CA | mean \|SHAP| | DeepSeek v2 surrogate | mean \|SHAP| |
|---|---:|---:|---:|---:|
...
![ML TreeSHAP vs LLM-surrogate SHAP](memos/figures/fig_shap_ml_vs_llm_compare.png){#fig-shap-ml-vs-llm}

Ride-share days (Q28) and employment drive true-CA prediction, while the persona agent's outputs are dominated by **geolocation and age** — over-weighting place/demographic cues relative to the mobility/employment mix that predicts the actual PRCA scores. Mean |SHAP| magnitudes are also ~7× smaller for the surrogate, so no single profile feature dominates the agent's scores.
...
Sex and student-status MAE gaps are small (≤ ~0.5 points). **Employment is the striking slice**: Full-Time error sits at 4.35–4.63 in every tier, while Part-Time and Other hover at 5.5–6.2. Signed error (predicted − true) is near zero for Full-Time (−0.01) but **−2.0 to −3.6 for Part-Time / Other** at every tier — the live agent systematically **under-predicts CA for non-full-time respondents** ([`docs/stereotyping_evaluation.md`](docs/stereotyping_evaluation.md)). This sharpens the RQ1 story: employment does not *shrink* overall MAE, but it is a real **calibration split** in the live model.
...
The primary (RQ1–RQ3), secondary (S1–S5), and focus (TF1–TF2) analyses share the same matched analytic cohort (**N = 241**) and the same information-tier logic, so they can be read as one measurement story. The table collects each question's headline statistic and bottom line.
...
| S1 · transit ↔ CA | Regular riders lower CA (group Δ −2.72, d −0.46, p = .0003) | CA and transit are descriptively related |
...
For **Posit Connect Cloud** [@quarto; @positconnect]: publish this repository [@psych755repo] with `_quarto.yml` as the primary file and `requirements.txt` present so Python dependencies install at build time. When private exports are absent, the manuscript loads committed full-cohort artifacts — participants, seeded secondary metrics, ML-vs-LLM metric tables (`artifacts/posit_full_cohort/ml_vs_llm/`), and the vLLM export tables including real stereotyping slices.

A **GitHub Pages** mirror (permanent portfolio URL) is built by [`.github/workflows/publish-gh-pages.yml`](.github/workflows/publish-gh-pages.yml) on pushes to `main` using the same committed-artifact path: [https://exios66.github.io/psych755-jjb/](https://exios66.github.io/psych755-jjb/). Enable **Settings → Pages → Source: GitHub Actions** once after merge. Posit Connect remains the interactive full-cohort deployment when File A/B/C and credentials are available.
