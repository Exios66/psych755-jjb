...
Group-CA MAE and signed error (predicted − ground truth) were sliced by Sex, Student status, and Employment status ([@cheng2023; @salewski2023]) from the committed DeepSeek-R1-Distill-Llama-8B v2 export (`exports/v2/psych755_vllm_deepseek_r1_distill_llama8b_v2_full_cohort_20260730_2213/tables/stereotyping_by_*.csv`). Slices cover all five v2 tiers (`demos` → `full`).
...
Table @tbl-ls-1 reports group-CA MAE by employment group across tiers.
...
Table @tbl-ls-2 reports the Sex slice.
...
Table @tbl-ls-3 reports the Student-status slice.
...
- v2/v3 pooled and per-tier results: [`vllm_v2_v3_evaluation.qmd`](vllm_v2_v3_evaluation.qmd)
- Band discrimination and surrogate SHAP for the same live agent: [`feature_predictive_power_ml_llm.qmd`](feature_predictive_power_ml_llm.qmd)
- Wave-2 demographics → transit (age-driven, relevant to `demos`-tier bias audits): [`demographics_predict_transit.qmd`](demographics_predict_transit.qmd)
