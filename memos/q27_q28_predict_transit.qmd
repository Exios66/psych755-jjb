...
**Companion memos:** [`rideshare_predicts_transit.qmd`](rideshare_predicts_transit.qmd) · [`transit_covariate_followups.qmd`](transit_covariate_followups.qmd) · parent [`geo_predicts_transit.qmd`](geo_predicts_transit.qmd)  
...
Using the Prolific↔Qualtrics matched analytic cohort (File A + File B joined to File C; base **n = 241** with complete PRCA and usable `Q26`), we define **regular transit** as `Q26` ∈ {`4–8 days a month`, `8 or more days a month`} (weekly-or-more). We then fit balanced ****Random Forest**s** ([@breiman2001]) with stratified 5-fold CV (`random_state=42`) using categorical one-hot encodings of the survey items in Table @tbl-q27q28–1:
...
**Short answer:** **`Q28`** recovers CV ROC-AUC = **0.762** (AP = **0.689**; balanced accuracy = **0.730**). **`Q27`** recovers AUC = **0.589**—matching the CA benchmark (0.590 [@mccroskey1970])—and the joint `Q27`+`Q28` forest reaches AUC = **0.761** (Δ = −0.001 vs `Q28` alone). Ride-share days, not transit-day intensity, carry the usable discrimination.

Figure @fig-q27q28-q27-q28-predicts-transit plots `Q27` and `Q28` prevalence and ROC curves, and Figure @fig-q27q28-q27-q28-predicts-transit-comparison contrasts their head-to-head AUCs against the geo/CA benchmarks.
...
**`Q27` (n = 239).** Respondents reporting more rides on a typical transit day are more often weekly+ riders, but most of the sample sits in the lowest intensity bin (Table @tbl-q27q28–2):
...
**`Q28` (n = 241).** Regular-transit prevalence rises steeply with ride-share days (Table @tbl-q27q28–3):
...
Table @tbl-q27q28–4 reports the stratified-CV performance for the `Q27`/`Q28` forests.
...
**Answered in follow-ups:** `Q28`'s lift after car conditioning is addressed by [`q28_conditioned_on_car.qmd`](q28_conditioned_on_car.qmd) (AUC 0.665 → 0.730 with `Q21`); the within-rider intensity question by [`q27_intensity_among_riders.qmd`](q27_intensity_among_riders.qmd) (best AUC ≈ 0.549 — no usable signal). CA-plus-mobility joint models and multiply-imputed head-to-heads complete the picture in [`ca_mobility_joint_predicts_transit.qmd`](ca_mobility_joint_predicts_transit.qmd) and [`mi_head_to_head.qmd`](mi_head_to_head.qmd).

**Formal write-up:** [`docs/secondary_rq_q27_q28_predict_transit.md`](../docs/secondary_rq_q27_q28_predict_transit.md)
