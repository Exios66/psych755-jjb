...
**Short answer:** Yes — car license/access exceeds geography and CA on CV ROC-AUC. Car license/access recovers CV ROC-AUC = **0.607**, above chance (0.500), the geo RF benchmark (**0.551**), and the CA RF benchmark (**0.590**). Access (`Q21`) dominates license (`Q20`).
...
Permutation importance ranks **Q21** (mean AUC drop = 0.182) above **Q20** (0.014). At a 0.5 threshold the forest is conservative on the positive class (precision = 0.77, recall = 0.34, F1 = 0.48), consistent with a sparse “no access” positive signal.
...
Ride-share frequency (`Q28`/`Q29`) recovers AUC = **0.745** ([`rideshare_predicts_transit.md`](rideshare_predicts_transit.md)). Employment status alone recovers AUC = **0.528** ([`employment_predicts_transit.md`](employment_predicts_transit.md)). Head-to-head: [`transit_covariate_followups.md`](transit_covariate_followups.md).
