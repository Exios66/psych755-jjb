...
Using the Prolific↔Qualtrics matched cohort for this project (File A + File B stacked joined to File C on Prolific ID / `Q0`; **252** matched rows; analytic **n = 241** with complete PRCA items), we asked whether ground-truth **group** and **interpersonal** CA scores (range 6–30) predict **regular** public-transit use, defined as `Q26` ∈ {`4-8 days a month`, `8 or more days a month`} (weekly-or-more). A balanced **Random Forest** (stratified 5-fold CV) was trained on the two CA subscales, with group-only and interpersonal-only ablations and a chance baseline (ROC-AUC = 0.50).
...
Permutation importance likewise ranks **group CA** above interpersonal CA. Combining both subscales improves AUC by about +0.03 over the better single-feature model.
...
Survey geolocation (latitude/longitude) was examined in a companion Random Forest (`secondary_rq_geo_transit_rf.ipynb`) and showed similarly modest AUC. Promising next features include car access / license (`Q20`/`Q21`), employment status, country or city density proxies, and ride-share frequency (`Q28`/`Q29`), alone and jointly with CA.
