...
Figures match the formal write-up [`docs/secondary_rq_ca_predicts_transit.md`](../docs/secondary_rq_ca_predicts_transit.md) and seeded CLI artifacts under `outputs/ca_transit_rf/` (`ca-personas ca-transit-rf --join inner --seed 42`).
...
Survey geolocation (latitude/longitude) was examined in a companion Random Forest (`secondary_rq_geo_transit_rf.ipynb`) and showed similarly modest AUC (**0.551**; country-only **0.549**). Promising next features include car access / license (`Q20`/`Q21`), employment status, country or city density proxies, and ride-share frequency (`Q28`/`Q29`), alone and jointly with CA.
