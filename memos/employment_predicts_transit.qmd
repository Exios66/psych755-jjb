...
Using the full matched analytic cohort (**n = 241**; 101 regular / 140 not regular), we evaluated **Employment status** (Full-Time / Part-Time / Other) as a single-feature **Random Forest** ([@breiman2001]) predicting weekly+ public transit (`Q26` threshold as in the geo and CA memos). Stratified 5-fold CV, `random_state=42`.

**Short answer:** Barely. Employment status yields CV ROC-AUC = **0.528** — only +0.028 above chance and **below** both the geo (**0.551**) and CA (**0.590**) benchmarks. It is not a competitive stand-alone predictor in this sample. Figure @fig-et-employment-predicts-transit shows prevalence by level and the ROC curve.
...
**Descriptive associations.** Table @tbl-et-1 reports employment prevalence.
...
**Random Forest (stratified CV).** Table @tbl-et-2 reports the CV ROC-AUCs.
