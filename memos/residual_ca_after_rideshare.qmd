...
**Parent open questions:** [`transit_riders_ca.qmd`](transit_riders_ca.qmd) · [`ca_scores_predict_transit.qmd`](ca_scores_predict_transit.qmd)

---

## Answer, Response, + Summary of Results

On `Q28`-complete rows (n = **241**), we (i) replicate the overall transit→CA [@mccroskey1970] Welch contrast, (ii) compare nested RFs ([@breiman2001]; CA only / `Q28` only / CA+`Q28`), and (iii) run within-`Q28`-stratum Welch tests.

**Short answer:** The **mean CA gap among regular riders remains real overall**, but for *prediction* CA adds almost nothing once `Q28` is present (**+0.021** AUC). Within `Q28` strata, CA differences are **heterogeneous**—sometimes reversed (Figure @fig-rc-residual-ca-q28-followup).

![Residual CA nested AUCs](figures/residual_ca_q28_followup_memo.png){#fig-rc-residual-ca-q28-followup}

### Overall transit → CA (replication)

Table @tbl-rc-1 reports the overall transit → CA Welch contrasts.
...
: Residual CA after controlling for `Q28`. {#tbl-rc-1}


### Nested predictive models

Table @tbl-rc-2 reports the nested predictive models.
...
Table @tbl-rc-3 reports the within-`Q28`-stratum contrasts.
...
: `Q28` level / Group CA Δ (reg − not) / Welch *p*. {#tbl-rc-3}
...
*Sources:* `ca-personas followup-experiments --experiments residual_ca_q28` · `outputs/followup_experiments/residual_ca_q28/` · formal write-up [`docs/secondary_rq_followup_experiments.md`](../docs/secondary_rq_followup_experiments.md)
