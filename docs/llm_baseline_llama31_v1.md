...
**Companion:** [Prompt version map](persona_prompt_versions.md) · [v2/v3 packaging](persona_prompt_efficiency.md) · [ML baselines](ml_baselines.md)

> This page is the published baseline for **prompt version 1** (five cumulative tiers: `demos` → `full`). Later packaging (v2 / v3.1) and ablation tiers (v3) are designed against these failure modes; they have not yet been re-run on GPU in this report.
