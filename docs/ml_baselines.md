...
1. **Adding context helps — modestly.** RF group MAE falls from 5.72 (`demos`) to 4.68 (`transit`), a ~1.0-point improvement on a 24-point scale.
2. **Geography and transit matter more than employment alone.** The largest RF gains appear when lat/long and transit items enter the feature set.
3. **RF beats KNN** at every tier on MAE for both subscales in this run.
4. **Band accuracy stays middling** (≈ 0.37–0.53). Tabular models recover some signal but do not pin the low/moderate/high band reliably from demographics + context alone.
5. **R² is near zero or negative** until the `transit` tier (group R² ≈ 0.06; interpersonal ≈ 0.10) — CA is only weakly linear in these covariates.
...
- Because even the best RF MAE remains ~4.3–4.7 points, persona prompts that land in that range are competitive with a strong tabular learner — not magical.
- Feature-importance diagnostics ([factor_feature_importance.md](factor_feature_importance.md)) show *which* covariates carry the tabular signal the LLM is being asked to use.
