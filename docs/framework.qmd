...
| Memo | [`memos/transit_riders_ca.qmd`](../memos/transit_riders_ca.qmd) |
...
| Memo | [`memos/geo_predicts_transit.qmd`](../memos/geo_predicts_transit.qmd) |
...
| Memo | [`memos/ca_scores_predict_transit.qmd`](../memos/ca_scores_predict_transit.qmd) |
...
### Secondary focus — transit prediction with mobility held out (TF1 / TF2)

| Piece | Detail |
|---|---|
| Hold-out | `Q26`–`Q29`, `Q20`, `Q21`, `Q19` excluded from features **and** persona text |
...
| Write-up | [`docs/secondary_rq_transit_focus.md`](secondary_rq_transit_focus.md) |
...
Cumulative information revealed to the model as **natural-language, second-person
digital-twin narratives** (AI Terrarium / ICA 2026 framing — “You are a …”), not
...
6. `v3_rideshare` — Q28 only (+ Q29 if used)
7. `v3_public_transit` — Q26 only (+ Q27 if used)
8. `v3_voice` — Q18.1 / Q19 open-text only (no structured transit)

Version map (v1 baseline → v2 packaging → v3 ablations): [`persona_prompt_versions.md`](persona_prompt_versions.md).  
Llama-3.1 full-cohort baseline (prompt v1): [`llm_baseline_llama31_v1.md`](llm_baseline_llama31_v1.md).  
Packaging details: [`persona_prompt_efficiency.md`](persona_prompt_efficiency.md).  
See [`prompts/system_prompt.md`](../prompts/system_prompt.md) and
[`prompts/examples/`](../prompts/examples/) for the system contract and sample prompts.
